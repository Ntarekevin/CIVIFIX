              ...
              <table className="w-full text-left">
                <thead>
                  <tr className="bg-gray-50 dark:bg-gray-800/50 border-b dark:border-gray-700">
                    <th className="px-6 py-4 text-xs font-bold text-gray-400 uppercase tracking-widest">Report Reference</th>
                    <th className="px-6 py-4 text-xs font-bold text-gray-400 uppercase tracking-widest text-right">Preview / Manage</th>
                  </tr>
                </thead>
                <tbody className="divide-y dark:divide-gray-800">
                  {reports.map(r => (
                    <React.Fragment key={r.id}>
                      <tr 
                        className="hover:bg-gray-50 dark:hover:bg-gray-800/20 transition-colors cursor-pointer"
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-4">
                            ...
                            <div className="space-y-0.5">
                              <p className="font-bold text-gray-900 dark:text-white uppercase tracking-tighter">ID {r.public_id || r.id}</p>
                              <p className="text-[10px] text-gray-500 font-medium uppercase">{new Date(r.created_at).toLocaleString()}</p>
                            </div>
                          </div>
                        </td>
                        <td className="px-6 py-4 text-right">
                           <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-gray-100 dark:bg-gray-800 text-[10px] font-bold text-gray-500 uppercase">
                              Click to Manage
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div className="space-y-4">
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        <span className={`px-2 py-1 rounded text-[10px] font-bold uppercase ${CATEGORY_COLORS[r.category] || 'bg-gray-100'}`}>
                                            #{r.category}
                                        </span>
                                        <span className="px-2 py-1 bg-gray-100 dark:bg-gray-800 rounded text-[10px] font-bold text-gray-500 uppercase italic">
                                            {r.city || "General Location"}
                                        </span>
                                    </div>
                                </div>
                                ...
                                <div className="space-y-4 border-l dark:border-gray-800 pl-8">
                                    <div className="grid grid-cols-2 gap-4">
                                        <div className="space-y-2">
                                            ...
                                            <select 
                                                ...
                                                className="w-full bg-white dark:bg-surface-dark border dark:border-gray-700 rounded-xl px-3 py-2 text-xs font-bold focus:ring-2 focus:ring-primary/20 outline-none transition-all dark:text-white"
                                            >
                                        ...
                                        <div className="space-y-2">
                                            <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Assign Official</label>
                                            <select 
                                                value={r.assigned_to || ''} 
                                    ...
                                    <div className="space-y-2 pt-2">
                                        <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Official Reply</label>
                                        <div className="flex gap-2">
                                            <input 
                                                type="text"
                                                placeholder="Type response..."
                                                className="flex-1 bg-white dark:bg-surface-dark border dark:border-gray-700 rounded-xl px-4 py-2 text-xs outline-none focus:ring-2 focus:ring-primary/20 transition-all dark:text-white"
                                                id={`reply-${r.id}`}
                                            />
                                            <button 
                                                onClick={async () => {
